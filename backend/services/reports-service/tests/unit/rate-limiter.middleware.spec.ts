...
describe('rateLimiter', () => {
  ...
  it('should use socket fallback', () => {
    mw({ ip: undefined, socket: { remoteAddress: '1.2.3.4' } } as unknown as Request, mockRes(), next);
    expect(next).toHaveBeenCalled();
  });
  it('should reset after window', () => {
    const req = mockReq('10.4.0.6'); const res = mockRes();
