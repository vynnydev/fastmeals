...
export class BedrockAIService implements IAIService {
  ...
  async generateInsights(input: AIInsightsInput): Promise<AIInsightsResult> {
    ...
    try {
      ...
      if (isAnthropic) {
        aiText = await this.invokeAnthropic(prompt);
      } else {
        aiText = await this.invokeConverse(prompt);
      }
  ...
  // Anthropic Claude models (InvokeModel API)
  ...
  // Amazon Nova and other models (Converse API - universal)
  private async invokeConverse(prompt: string): Promise<string> {
