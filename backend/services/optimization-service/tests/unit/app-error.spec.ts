...
describe('AppError', () => {
  ...
  describe('static factories', () => {
    ...
    it('badRequest defaults empty details', () => { expect(AppError.badRequest('V', 'b').details).toEqual([]); });
    ...
    it('unprocessable defaults empty', () => { expect(AppError.unprocessable('U', 'u').details).toEqual([]); });
    it('tooManyRequests 429 default', () => { const e = AppError.tooManyRequests(); expect(e.statusCode).toBe(429); expect(e.message).toBe('Limite de requisições atingido'); });
    it('tooManyRequests custom', () => { expect(AppError.tooManyRequests('X').message).toBe('X'); });
    it('internal 500 default', () => { const e = AppError.internal(); expect(e.statusCode).toBe(500); expect(e.message).toBe('Erro interno do servidor'); });
