...
import { updateProductSchema, productIdSchema } from '../infrastructure/http/validators/product.validator';
import { AppError } from '../infrastructure/http/errors/app-error';
import { ZodError } from 'zod';

let useCase: UpdateProductUseCase | null = null;

function getUseCase(): UpdateProductUseCase {
  if (!useCase) {
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    const product = await getUseCase().execute(id, validated);
