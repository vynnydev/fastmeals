...
export function AnimatedNumber({
  ...
  duration = 1500, delay = 0, className = '',
}: {
...
export function AnimatedCurrency({
  value, duration = 1500, delay = 0, className = '',
}: {
  value: number; duration?: number; delay?: number; className?: string
}) {
  const count = useAnimatedCounter(value, { duration, delay, decimals: 2 })
  const formatted = new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(count)
