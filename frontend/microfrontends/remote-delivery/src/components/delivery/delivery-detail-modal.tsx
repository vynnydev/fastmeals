...
import { Phone, Truck, MapPin, Star, Package, Clock } from 'lucide-react'
      ...
      <DialogContent className="max-w-md bg-card border-border">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
              <Star className={cn('h-4 w-4', rating ? 'text-amber-400 fill-amber-400' : 'text-muted-foreground')} />
              <div>
                <p className="text-xs text-muted-foreground">Avaliação</p>
                <p className="text-sm font-medium">{rating ? Number(rating).toFixed(1) : 'Sem avaliação'}</p>
              </div>
            </div>
          </div>
            ...
            <>
              ...
              <div className="flex items-center gap-3 p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/10">
                ...
                <div>
                  ...
                  <p className="text-sm font-mono">{Number(lat).toFixed(4)}, {Number(lng).toFixed(4)}</p>
                </div>
              </div>
            </>
          )}
