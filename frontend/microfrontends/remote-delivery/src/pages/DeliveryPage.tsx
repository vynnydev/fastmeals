...
import type { DeliveryPerson, Assignment, Order } from '@/types'
...
import {
  Plus, Search, Users, Zap, RefreshCcw, Filter,
  CheckCircle, XCircle, MapPin, Route,
  Truck, ArrowRight, Loader2, LayoutGrid, Columns3,
} from 'lucide-react'
...
export default function DeliveryPage() {
  ...
  // Data fetching
  const { data: deliveryData, error, isLoading, mutate } = useDelivery()
  ...
  const handleRunOptimization = async () => {
    ...
    try {
      ...
      toast.success(`${result.assignments?.length || 0} atribuições sugeridas em ${result.executionTimeMs}ms`)
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Não foi possível executar a otimização')
    } finally {
  ...
  const handleAcceptAssignment = async (assignment: Assignment) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Erro ao aplicar atribuição')
    } finally {
  ...
  const handleRejectAssignment = (assignment: Assignment) => {
    ...
    toast.info('Atribuição rejeitada')
  }
  ...
  const handleEditDriver = (driver: DeliveryPerson) => { setSelectedPerson(driver); setFormModalOpen(true) }
  const handleDeleteDriver = (driver: DeliveryPerson) => { setSelectedPerson(driver); setDeleteModalOpen(true) }
  const handleViewDriver = (driver: DeliveryPerson) => { setSelectedPerson(driver); setDetailModalOpen(true) }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Não foi possível remover o entregador')
    } finally {
      setIsDeleting(false)
    }
  }
  ...
  const handleAssignOrder = (driver: DeliveryPerson) => {
    toast.info(`Selecione um pedido para atribuir a ${driver.name}`)
  }
  ...
  if (error) {
    return (
      <div className="space-y-6">
        ...
        <ErrorState title="Erro ao carregar dados" message="Não foi possível carregar as informações de entrega." onRetry={() => mutate()} />
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {canWrite && (
            <Button onClick={() => { setSelectedPerson(null); setFormModalOpen(true) }} className="gold-gradient text-primary-foreground">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-muted/30">
          <TabsTrigger value="drivers" className="gap-2"><Users className="h-4 w-4" /> Entregadores</TabsTrigger>
          <TabsTrigger value="optimization" className="gap-2"><Zap className="h-4 w-4" /> Otimização</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="drivers" className="space-y-4">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar entregadores..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 bg-background" />
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center border border-border rounded-md">
                <Button variant="ghost" size="icon" className={cn('h-9 w-9 rounded-r-none', driversView === 'cards' && 'bg-muted')} onClick={() => setDriversView('cards')}>
                  <LayoutGrid className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="icon" className={cn('h-9 w-9 rounded-l-none', driversView === 'kanban' && 'bg-muted')} onClick={() => setDriversView('kanban')}>
          ...
          {driversView === 'kanban' ? (
            ...
            <EmptyState
              ...
              description={search || statusFilter !== 'all' ? 'Tente ajustar os filtros de busca' : 'Adicione entregadores para começar'}
            />
          ) : (
        ...
        <TabsContent value="optimization" className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <Card className="p-4 bg-card/50 border-border/50">
              <div className="flex items-center gap-3">
                <div className="rounded-lg p-2 bg-amber-500/10"><Route className="h-5 w-5 text-amber-400" /></div>
                <div>
            ...
            <Card className="p-4 bg-card/50 border-border/50">
              <div className="flex items-center gap-3">
                <div className="rounded-lg p-2 bg-emerald-500/10"><Users className="h-5 w-5 text-emerald-400" /></div>
                <div>
            ...
            <Card className="p-4 bg-card/50 border-border/50">
              <div className="flex items-center gap-3">
                <div className="rounded-lg p-2 bg-blue-500/10"><MapPin className="h-5 w-5 text-blue-400" /></div>
                <div>
              ...
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {deliveryData?.deliveryPersons
                  ...
                  .map(driver => {
                    ...
                    const vType = driver.vehicleType || driver.vehicle_type
                    return (
                      <Card key={driver.id} className="p-3 bg-card/30 border-border/50">
                        <div className="flex items-center gap-3">
                          <div className="h-8 w-8 rounded-full bg-emerald-500/10 flex items-center justify-center flex-shrink-0">
                            <span className="text-sm font-semibold text-emerald-400">{driver.name.charAt(0)}</span>
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className="text-sm font-medium truncate">{driver.name}</p>
                            <div className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{vType === 'motorcycle' ? '🏍️' : vType === 'bicycle' ? '🚲' : '🚗'} {vType}</span>
                            </div>
                          </div>
                          <div className="text-right flex-shrink-0">
                            {lat && lng ? (
                              <div className="flex items-center gap-1 text-xs text-emerald-400"><MapPin className="h-3 w-3" /><span>GPS ativo</span></div>
                            ) : (
                              <div className="flex items-center gap-1 text-xs text-muted-foreground"><MapPin className="h-3 w-3" /><span>Sem GPS</span></div>
                            )}
                            <Badge className="mt-1 bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-xs">Livre</Badge>
                          </div>
                        </div>
                      </Card>
                    )
          ...
          <div className="flex flex-col items-center gap-4 py-6">
            ...
            {canWrite && (
              ...
              >
                {isOptimizing ? (
                  <><Loader2 className="h-5 w-5 animate-spin" /> Calculando atribuição ótima...</>
                ) : (
                  <><Zap className="h-5 w-5" /> Sugerir Atribuição Otimizada</>
                )}
              </Button>
            )}
          </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="p-4 rounded-lg bg-destructive/5 border border-destructive/20">
                        ...
                        <p className="text-2xl font-bold text-foreground">~{(optimizationResult.totalDistanceKm * 2.2).toFixed(1)} km</p>
                        <p className="text-xs text-muted-foreground mt-1">Atribuição greedy individual</p>
                      </div>
                      <div className="p-4 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                        ...
                        <p className="text-2xl font-bold text-foreground">{optimizationResult.totalDistanceKm.toFixed(1)} km</p>
                        <p className="text-xs text-muted-foreground mt-1">Atribuição globalmente ótima</p>
                      </div>
                    </div>
                          ...
                          <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                              {isAccepted ? (
                                ...
                                <>
                                  <Button size="sm" variant="outline" className="text-destructive hover:text-destructive" onClick={() => handleRejectAssignment(assignment)} disabled={isApplying}>
                                    <XCircle className="h-4 w-4" />
                                  </Button>
                                  <Button size="sm" onClick={() => handleAcceptAssignment(assignment)} disabled={isApplying} className="gap-1 bg-emerald-600 hover:bg-emerald-700 text-white">
                    ...
                    {optimizationResult.unassigned.map((item) => (
                      <Card key={item.orderId} className="p-3 bg-amber-500/5 border-amber-500/20">
                        <div className="flex items-center gap-3">
                          ...
                          <Badge variant="outline" className="text-amber-400 border-amber-500/30 text-xs">{item.reason}</Badge>
                        </div>
                      </Card>
                    ))}
                  </div>
      ...
      {deleteModalOpen && selectedPerson && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-lg p-6 max-w-md w-full mx-4 space-y-4">
            <h3 className="text-lg font-semibold text-foreground">Remover Entregador</h3>
            <p className="text-sm text-muted-foreground">
