...
export function OrderTable({
  ...
  const getCustomerName = (order: Order): string => {
    return order.customerName || order.customer_name || 'Cliente'
  }
  ...
  const getDeliveryPersonId = (order: Order): string | undefined => {
    return order.deliveryPersonId || order.delivery_person_id || undefined
  }
  ...
  useEffect(() => {
    const loadDriverNames = async () => {
      const driverIds = [...new Set(
        orders
          .map(o => o.deliveryPersonId || o.delivery_person_id)
          .filter(Boolean)
      )] as string[]
